...
class Config:
    ...
    select_feature_cols = ['Basic_Demos-Age', 'Basic_Demos-Sex', 'Physical-Height', 'Physical-Weight', 'FGC-FGC_CU', 'FGC-FGC_GSND', 'FGC-FGC_GSD', 'FGC-FGC_PU', 'BIA-BIA_Activity_Level_num', 'SDS-SDS_Total_Raw', 'SDS-SDS_Total_T', 'PreInt_EduHx-computerinternet_hoursday', 'light_max']
...
def cv_models(Datasets, feature_cols):
    ...
    output_dir = os.path.join(output_dir['MODEL_CHECKPOINT_DIR'], 'lgb', 'plain')
    ...
    for i, k_fold_seed in enumerate(Config.k_fold_seeds):
        ...
        def objective(trial):
            params_tuning = {
                ...
                'bagging_fraction': trial.suggest_float('bagging_fraction', 0.35, 1.0),
            ...
            for k in range(Config.n_splits):
                ...
                model.fit(
                    ...
                    eval_metric='mse',       # 評価指標を指定
                    callbacks=[lgb.early_stopping(50, verbose=False)]
                )

                # 検証
                y_preds = model.predict(X_va, num_iteration=model.best_iteration_)
